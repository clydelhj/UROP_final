...
class VirtualStainingCFMLitModule(CFMLitModule):
    """CFM for unpaired virtual staining (e.g. frozen -> H&E).

    The datamodule returns (source, target) batches. source (x0) is the frozen
    distribution, target (x1) is the H&E distribution. They are randomly paired
    within each batch; OT-CFM finds the optimal coupling.
    """

    def step(self, batch: Any, training: bool = False):
        x0, x1 = batch[0], batch[1]  # frozen patches, H&E patches

        if self.ot_sampler is not None:
            x0, x1 = self.ot_sampler.sample_plan(x0, x1)

        t_select = torch.zeros(1, device=x0.device)
    ...
    def eval_step(self, batch: Any, batch_idx: int, prefix: str):
        if prefix == "val":
            ...
            loss = mse + reg
            self.log_dict(
                {f"{prefix}/loss": loss, f"{prefix}/mse": mse},
                on_step=False,
                on_epoch=True,
                sync_dist=True,
            )
