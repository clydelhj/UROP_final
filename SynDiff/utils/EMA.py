...
class EMA(Optimizer):
    ...
    def state_dict(self):
        # EMA.__init__ never calls super().__init__(), so the base Optimizer
        # state_dict path (which reads self._optimizer_state_dict_pre_hooks
        # on newer PyTorch) crashes. Delegate to the wrapped optimizer —
        # EMA tensors live in self.optimizer.state[p]['ema'] and are
