...
from dataset import H5PatchDataset
...
def sample_from_model(coefficients, generator, n_time, x_init, T, opt):
    # x_init: (B, 6, H, W) = [3ch noisy target | 3ch source] for RGB
    x = x_init[:, :3, :]
    source = x_init[:, 3:, :]
    with torch.no_grad():
        for i in reversed(range(n_time)):
            ...
            # NCSNpp outputs num_channels=6 channels; first 3 are the predicted x0
            x_new = sample_posterior(coefficients, x_0[:, :3, :], x, t)
            x = x_new.detach()
...
def train_syndiff(rank, gpu, args):
    ...
    # Train: load all JPEG bytes into RAM (~30 GB per process).
    # Val:   stream from disk to avoid doubling RAM usage.
    dataset = H5PatchDataset(args.input_path, phase='train', augment=True, load_to_ram=True)
    dataset_val = H5PatchDataset(args.input_path, phase='val', augment=False, load_to_ram=False)
    ...
    train_sampler = torch.utils.data.distributed.DistributedSampler(dataset,
                                                                    ...
                                                                    rank=rank)
    # num_workers=0: each DDP process already holds ~30 GB of JPEG bytes in RAM;
    # forked workers would duplicate that memory and exceed system RAM.
    ...
    # +1: outer loop is range(init_epoch, num_epoch+1), so epoch index can equal num_epoch.
    val_l1_loss=np.zeros([2,args.num_epoch+1,args.val_steps])
    val_psnr_values=np.zeros([2,args.num_epoch+1,args.val_steps])
    ...
    # args.num_channels=6 (3ch noisy target + 3ch coarse translation input)
    gen_diffusive_1 = NCSNpp(args).to(device)
    gen_diffusive_2 = NCSNpp(args).to(device)
    #networks performing translation (RGB: 3ch in, 3ch out)
    args.num_channels = 3
    gen_non_diffusive_1to2 = backbones.generator_resnet.define_G(input_nc=3, output_nc=3, netG='resnet_6blocks', gpu_ids=[gpu])
    gen_non_diffusive_2to1 = backbones.generator_resnet.define_G(input_nc=3, output_nc=3, netG='resnet_6blocks', gpu_ids=[gpu])

    # nc=6: discriminator concatenates x_t (3ch) and x_{t+1} (3ch) before start_conv
    ...
    disc_non_diffusive_cycle1 = backbones.generator_resnet.define_D(input_nc=3, ndf=args.ngf, gpu_ids=[gpu])
    disc_non_diffusive_cycle2 = backbones.generator_resnet.define_D(input_nc=3, ndf=args.ngf, gpu_ids=[gpu])
    ...
    if rank == 0:
        ...
        _log_file = open(os.path.join(exp_path, 'train.log'), 'a', buffering=1)
    ...
    if args.resume:
        ...
        # iteration to skip to within init_epoch (0 for old checkpoints / end-of-epoch saves)
        ...
        # Re-sync T_max with current args.num_epoch. Older checkpoints saved with
        # num_epoch=0 baked T_max=0 into scheduler state, which makes
        # CosineAnnealingLR.get_lr() divide by zero on the next step.
        for _sched in (scheduler_gen_diffusive_1, scheduler_gen_diffusive_2,
                       scheduler_gen_non_diffusive_1to2, scheduler_gen_non_diffusive_2to1,
                       scheduler_disc_diffusive_1, scheduler_disc_diffusive_2,
                       scheduler_disc_non_diffusive_cycle1, scheduler_disc_non_diffusive_cycle2):
            _sched.T_max = args.num_epoch
        global_step = checkpoint['global_step']
        print("=> loaded checkpoint (epoch {}, iteration {}, global_step {})"
                  .format(checkpoint['epoch'], init_iteration, global_step))
    else:
        global_step, epoch, init_epoch, init_iteration = 0, 0, 0, 0
    
    
    for epoch in range(init_epoch, args.num_epoch+1):
        train_sampler.set_epoch(epoch)
        epoch_start = time.time()
        # On the first epoch after --resume, fast-forward through batches we
        # already trained on. DistributedSampler is deterministic given the
        # epoch seed, so iter N here loads the same shard as iter N originally.
        skip_iters = init_iteration if epoch == init_epoch else 0
        if rank == 0 and skip_iters > 0:
            log(f'Resuming epoch {epoch} at iteration {skip_iters} (fast-forwarding data loader).')

        pbar = tqdm(data_loader,
                    desc=f'Epoch {epoch:03d}',
                    disable=(rank != 0),
                    dynamic_ncols=True)
        for iteration, (x1, x2) in enumerate(pbar):
            ...
            x1_pos_sample = sample_posterior(pos_coeff, x1_0_predict_diff[:, :3, :], x1_tp1, t1)
            x2_pos_sample = sample_posterior(pos_coeff, x2_0_predict_diff[:, :3, :], x2_tp1, t2)
            ...
            #L1 loss
            errG1_L1 = F.l1_loss(x1_0_predict_diff[:, :3, :], real_data1)
            errG2_L1 = F.l1_loss(x2_0_predict_diff[:, :3, :], real_data2)
            ...
            if rank == 0:
                pbar.set_postfix({
                    'G-Cyc':  f'{errG_cycle.item():.4f}',
                    'G-L1':   f'{errG_L1.item():.4f}',
                    'G-Adv':  f'{errG_adv.item():.4f}',
                    'Gc-Adv': f'{errG_cycle_adv.item():.4f}',
                    'G':      f'{errG.item():.4f}',
                    'D':      f'{errD.item():.4f}',
                    'Dc':     f'{errD_cycle.item():.4f}',
                    'step':   global_step,
                })
                if iteration % 100 == 0:
                    log(
                        f'Epoch {epoch:03d} | Iter {iteration:06d} | Step {global_step:07d} | '
                        f'G-Cycle: {errG_cycle.item():.4f} | G-L1: {errG_L1.item():.4f} | '
                        f'G-Adv: {errG_adv.item():.4f} | Gc-Adv: {errG_cycle_adv.item():.4f} | '
                        f'G-Sum: {errG.item():.4f} | D: {errD.item():.4f} | Dc: {errD_cycle.item():.4f}'
                    )
                if (args.save_content and args.save_content_every_steps > 0
                        and global_step % args.save_content_every_steps == 0):
                    log(f'[save] step={global_step} starting (mid-epoch, epoch {epoch}, iter {iteration})')
                    _save_t0 = time.time()
                    log(f'[save] building content dict')
                    content = {'epoch': epoch, 'iteration': iteration + 1, 'global_step': global_step, 'args': args,
                    ...
                    log(f'[save] dict built in {time.time()-_save_t0:.1f}s, calling torch.save')
                    tmp_path = os.path.join(exp_path, 'content.pth.tmp')
                    _t1 = time.time()
                    torch.save(content, tmp_path)
                    log(f'[save] torch.save(content) done in {time.time()-_t1:.1f}s, renaming')
                    os.replace(tmp_path, os.path.join(exp_path, 'content.pth'))
                    log(f'[save] rename content.pth done')

                    # EMA swap mirrors the end-of-epoch checkpoint logic so the saved
                    # weights are inference-ready (same as gen_*_{epoch}.pth at epoch end).
                    if args.use_ema:
                        log(f'[save] EMA swap (forward)')
                        ...
                        log(f'[save] EMA swap (forward) done')
                    for _name, _model in (
                        ...
                        log(f'[save] saving {_name}_latest.pth')
                        _final = os.path.join(exp_path, f'{_name}_latest.pth')
                        _tmp = _final + '.tmp'
                        _ts = time.time()
                        torch.save(_model.state_dict(), _tmp)
                        os.replace(_tmp, _final)
                        log(f'[save]   {_name}_latest.pth done in {time.time()-_ts:.1f}s')
                    if args.use_ema:
                        log(f'[save] EMA swap (back)')
                        optimizer_gen_diffusive_1.swap_parameters_with_ema(store_params_in_ema=True)
                        optimizer_gen_diffusive_2.swap_parameters_with_ema(store_params_in_ema=True)
                        optimizer_gen_non_diffusive_1to2.swap_parameters_with_ema(store_params_in_ema=True)
                        optimizer_gen_non_diffusive_2to1.swap_parameters_with_ema(store_params_in_ema=True)
                        log(f'[save] EMA swap (back) done')
                    log(f'[save] step={global_step} complete in {time.time()-_save_t0:.1f}s')
        ...
        if rank == 0:
            ...
            #
            pred1 = torch.cat((real_data2, pred1, gen_non_diffusive_1to2(pred1), fake_sample2_tilda[:, :3, :]),axis=-1)
            ...
            #            
            pred2 = torch.cat((real_data1, pred2, gen_non_diffusive_2to1(pred2), fake_sample1_tilda[:, :3, :]),axis=-1)
        ...
        if rank == 0:
            ...
            log(
                ...
                f'Val frozen->HE PSNR: {np.nanmean(val_psnr_values[0, epoch, :]):.4f} | '
                f'Val HE->frozen PSNR: {np.nanmean(val_psnr_values[1, epoch, :]):.4f}\n'
            )
...
if __name__ == '__main__':
    ...
    parser.add_argument('--num_channels', type=int, default=6,
                            help='input channels to NCSNpp diffusion generator: '
                                 '3ch noisy target + 3ch coarse source = 6 for RGB')
    ...
    parser.add_argument('--save_content_every_steps', type=int, default=0,
                        help='also save content.pth every N global steps (0 = disabled). '
                             'Enables mid-epoch resume when an epoch is too long to complete.')
    ...
    parser.add_argument('--max_steps', type=int, default=0,
                        help='stop training after this many global steps (0 = run for num_epoch epochs)')
    parser.add_argument('--val_steps', type=int, default=200,
                        help='number of val batches to evaluate per epoch')
