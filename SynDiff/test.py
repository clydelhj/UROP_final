...
def sample_and_test(args):
    ...
    phase = 'test'
    dataset = H5PatchDataset(args.input_path, phase='test', augment=False, load_to_ram=False)
    ...
    for iteration, (x , y) in enumerate(data_loader): 
        ...
        print(str(iteration))
        # save side-by-side: [source | generated | real_target]
    ...
    print('frozen->HE PSNR: {:.4f}'.format(np.nanmean(loss1)))
    np.save('{}/psnr_values_frozen_to_HE.npy'.format(save_dir), loss1)

    print('HE->frozen PSNR: {:.4f}'.format(np.nanmean(loss2)))
    np.save('{}/psnr_values_HE_to_frozen.npy'.format(save_dir), loss2)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--num_channels', type=int, default=6,
                            help='input channels to NCSNpp: 3ch noisy + 3ch source = 6 for RGB')
