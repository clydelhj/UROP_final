...
INPUT_PATH="/csproject/fyp25_gch6/juwon/lung_img_patches"    # directory containing train_A/, train_B/, val_A/, val_B/
OUTPUT_PATH="$(dirname "$0")/output"                         # saves to SynDiff/output/ alongside this script
EXP_NAME="syndiff_frozen_he"

# System /usr/local/cuda points to CUDA 10.1 (too old for cu118 PyTorch's JIT
# extension build). Force the build to use the matching CUDA 11.8 toolkit.
...
python3 train.py \
    ...
    --save_content_every_steps 3000 \
    --save_ckpt_every 1 \
    --max_steps 75000 \
    --val_steps 5000 \
    --num_process_per_node 6 \
    --local_rank 0 \
    --port_num 6021 \
